...
public class LatexTable {
    ...
    public static String ResponseTimeToString(Solution[] solutions, String[] operations) {
        ...
        result += "\\begin{tabular}{" + Util.Repeat("c", solutions.length + 2) + "}\n";
        ...
        for (String operation : crudKeys) {
            ...
            for (String table : crud.get(operation)) {
                ...
                for (int i = 0; i < solutions.length; ++i) {
                    ...
                    Util.Divide(times, numberOfOperations(table));
                    result += DF.format(Util.Mean(times)) + " ("
                            + DF.format(Util.StDev(times)) + ")";
                    if (i < solutions.length - 1) {
                        result += " & ";
                    }
                }
        ...
        for (String operation : crudKeys) {
            ...
            for (String table : crud.get(operation)) {
                ...
                for (int i = 0; i < solutions.length; ++i) {
                    ...
                    result += DF.format(Util.Mean(throughput))
                            + " (" + DF.format(Util.StDev(throughput)) + ")";
                    if (i < solutions.length - 1) {
                        result += " & ";
                    }
                }
    ...
    public static String RatioToString(Solution[] solutions, String[] operations) {
        ...
        for (String operation : crudKeys) {
            ...
            for (String table : crud.get(operation)) {
                ...
                Util.Divide(baselineTimes, numberOfOperations(table));

                result += DF.format(Util.Mean(baselineTimes)) + " & ";
                for (int i = 1; i < solutions.length; ++i) {
                    Solution s = solutions[i];

                    List<Double> times = new ArrayList<Double>(s.getTimesFor(operation + "_" + table));
                    Util.Divide(times, numberOfOperations(table));
                    result += DF.format(Util.Mean(times) / Util.Mean(baselineTimes));
                    if (i < solutions.length - 1) {
                        result += " & ";
                    }
                }
    ...
    public static String ThroughputRatioToString(Solution[] solutions, String[] operations) {
        ...
        for (String operation : crudKeys) {
            ...
            for (String table : crud.get(operation)) {
                ...
                result += DFB.format(Util.Mean(baselineThrouhput)) + " & ";
                for (int i = 1; i < solutions.length; ++i) {
                    Solution s = solutions[i];
                    ...
                    result += DF.format(Util.Mean(throuhput) / Util.Mean(baselineThrouhput));
